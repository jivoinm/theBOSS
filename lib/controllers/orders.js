...
exports.updateOrder = function(req,res){
    if(req.user){
        ...
        _.find(order.forms, function(form){
            isInProgress = _.find(form.tasks, function(task){
                return task.status && task.status !=='finished';
            }) != null 
        });

        order.status = isInProgress && order.status !=='blocked' ? 'in progress' : 
                        (order.status ==='blocked' ? 'blocked' : 'finished'); 
