...
exports.comments = function(req,res){
    var query = {owner: req.user.owner};
//    db.orders.aggregate({$unwind: "$comments"}, {$sort: {"comments.created_on":1}}, {$group: {_id: "$_id",comments: {$push
//"$comments"}}})
    Order.aggregate(
        {$unwind: "$comments"}, 
...
exports.updateOrder = function(req,res){
    if(req.user){
        ...
        FindOrder(req, function (err, orderFound) {
            if(err) res.json(400,err);
            for (var attributename in order){
                orderFound[attributename] = order[attributename];
            }
            orderFound.save(function (order) {
                return res.send(order);
            });
        });
    }
