...
function QueryOrders(queryOrders, sort, page, limit, res) {
    var query = Order.find(queryOrders)
        .select('customer.name po_number createdBy last_updated_by created_on last_updated_on date_required status services')
        .sort(sort);
    if(page){
...
exports.updateOrder = function(req,res){
    if(req.user){
        ...
        order.status = finished ? 'finished' : order.status; 
