...
exports.tasks = function(req,res){
    Order.find({
        ...
        .select('customer po_number createdBy forms.form_name forms.tasks date_required')
...
exports.updateTaskStatus = function (req, res){
    Order.update({ _id: req.params.id, owner: req.user.owner, 'forms': {$elemMatch: {'tasks._id': req.params.taskId}}},
        {$set: {'forms.$.tasks.$.status': req.params.status}},
        function (err){
            if(err) res.json(400,err);
            FindOrder(req, function (err, order){
                if (err) return res.json(400, err);
