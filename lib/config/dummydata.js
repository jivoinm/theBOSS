                        ...
                        new Array(100)
                            ...
                            .map(function(item, i){
                                ...
                                Customer.create({owner: 'DelPriore', name: 'Customer'+i}, function (err, Customer) {
                                    var tomorrow = moment().add('days', i);
                                    var req_date = new Date(tomorrow);
                                    //console.log('finished populating Customer'+i, 'req_date='+req_date);
                                    Order.find({}).remove(function () {
                                        var order = {
                                            owner: 'DelPriore',
                                            customer: Customer,
                                            po_number: i,
                                            created_by: (i % 2 == 0  ? user1 : user2),
                                        ...
                                        Order.create(order, function (err,new_order) {
                                            ...
                                            function calendarDetailFromOrder(order) {
                                                ...
                                                console.log(order, 'calendarDetailFromOrder: start',start,'End',requiredDate);
                                                return {
                                                    owner: 'DelPriore',
                                                    title: order.po_number + '-' +Customer.name +' - '+(i % 2 == 0  ? user1.name : user2.name),
                                                    //details: getOrderDetails(order),
                                                    details: 'http://localhost:9000/api/orders/'+order._id,
                                                    start: start,
                                                    end: requiredDate,
                                                    color: 'green',
                                                    allDay: true
                                            ...
                                            Calendar.create(calendarDetailFromOrder(new_order), function(err,cal){

                                                console.log(err, 'finished populating calendar', cal.title);
                                            });
                                        });
                                    });
                                });
