...
OrderSchema.methods = {
            ...
            projects.toObject().forEach(function (proj) {
                if(proj.project){
                    ...
                    tasks.push({
                        ...
                        tasks: project_tasks.map(function (task) {
    ...
    getProjects: function (cb) {
        ...
        self.constructor.populate(self.projects, select, function (err, servProjects) {
            servProjects.toObject().forEach(function (proj) {
                if(proj.project){
                    var project_fields = proj.project.fields.sort(function (a, b) {
                    ...
                    projects.push({
                            project: proj.project.name,
                            fields: project_fields.map(function (field) {
                                            var order_field_values = proj.field_values.filter(function (element) {
                                                return field.field_title in element;
                                            });

                                            return {field: field, field_value: order_field_values.length > 0 ? order_field_values[0][field.field_title] : ''};
                                        })
                        });
                }
            });
