    ...
    getProjects: function (cb) {
        ...
        self.constructor.populate(self.projects, select, function (err, projects) {
            projects.toObject().forEach(function (proj) {
                ...
                projects.push(project_fields.map(function (field) {
                    var order_field_values = proj.field_values.filter(function (element) {
                        return element.field_title === field.field_title;
                    });

                    return merge_options(field, order_field_values.length > 0 ? order_field_values[0] : {});
                }));
            });
            cb(projects);
        });
